...
function M.move_to_monitor(direction)
    return function()
        if hl.get_monitor(direction) then
...
function M.disconnect_bluetooth()
    hl.exec_cmd([[bluetoothctl devices Connected | awk '{print $2}' | xargs -rn1 bluetoothctl disconnect]])
end
